...
#processing already on the HiVT adapter
...
def main(cfg: DictConfig):
    ...
    print('preprocess line 43')
    train(entrypoint, config)
    breakpoint() #until this breakpoint, saves the preprocess file for the first subset, good, but with argo outputs and not HIVT adapter
    print('preprocess line 45')
    val(entrypoint, config)
    print('preprocess line 47')
...
def preprocess_dataset(dataset: Entrypoint, config: Config, mode: str = 'train'):  # FIXME Use split enum
    ...
    if config.hydra.data_features.dataset_name == "argo":
        num_samples_total_dataset = dataset.dataset_adapter.number_of_samples_in_complete_dataset
    if config.hydra.data_features.dataset_name == "Argo":
        #num_samples_total_dataset = dataset.dataset_adapter.number_of_samples_in_complete_dataset
        num_samples_total_dataset= 205943
    if num_samples_total_dataset is None:  # Save pkl file at once
        with open(save, 'wb') as f:
            #store = TemporalData(**store)
            #root_preprocess= config.hydra.data.root_preprocess
            torch.save(store,  os.path.join(f, str(store[0]['seq_id']) + '.pt'))
        ...
        for i in range(len(store)):
            ...
            #save_file_str = f"{save[:-2]}_{total_idx:07}"
            #with open(save_file_str, 'wb') as f:
                #store[i] = TemporalData(**store[i])
            root_preprocess= config.hydra.data.root_preprocess
            torch.save(store[i],  os.path.join(root_preprocess, str(store[i]['seq_id']) + '.pt')
                  )
